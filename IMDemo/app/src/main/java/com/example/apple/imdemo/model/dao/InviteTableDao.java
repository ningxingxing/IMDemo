...
import com.example.apple.imdemo.model.db.DBHelper;
...
public class InviteTableDao {
    ...
    public void addInvitation(InvationInfo invationInfo) {
        ...
        values.put(InviteTable.COL_RESON, invationInfo.getReason());//原因
        values.put(InviteTable.COL_STATUS, invationInfo.getStatus().ordinal());//状态

        UserInfo user = invationInfo.getUserInfo();
        if (user != null) {
            values.put(InviteTable.COL_USER_HXID, invationInfo.getUserInfo().getHxid());
            values.put(InviteTable.COL_USER_NAME, invationInfo.getUserInfo().getName());
        } else {//群组
    ...
    public List<InvationInfo> getInvitations() {
        ...
        while (cursor.moveToNext()) {
            ...
            invationInfo.setReason(cursor.getString(cursor.getColumnIndex(InviteTable.COL_RESON)));
            ...
            if (groupId == null) {//联系人的邀请信息
                ...
                groupInfo.setInvatePerson(cursor.getString(cursor.getColumnIndex(InviteTable.COL_USER_HXID)));//邀请人信息
    ...
    public void updateInvitationStatus(InvationInfo.InvitationStatus invitationStatus, String hxId) {
        ...
        db.update(InviteTable.TAB_NAME, values, InviteTable.COL_USER_HXID + "=?", new String[]{hxId});//根据环信id 查找
